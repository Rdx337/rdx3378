...
def chk():
  ...
  try: 
    ...
    if id in httpCaht: 
      ...
      print('\033[92m[•]\033[97mMY ID :  Md SABBIR  BHAI [ SABBIR-BHAI ]')
...
def passwrd():
	...
	print(f' Results OK Saved In : SABBIR_OK.txt')
...
def crack(idf,pwv):
	global loop,ok,cp
	#sys.stdout.write(f'\r \33[1;94m[SABBIR\33[1;93m]\033[1;97mOK-\033[38;5;46m%s'%(H,loop,len(ok))),
	bo = random.choice([m,k,h,b,u,x])
	sys.stdout.write(f"\r \33[1;95m[SABBIR]  {h}[{kk}{loop}/{len(id)}{h}] {h}[OK] {h}[{ok}] {h}[{'{:.0%}'.format(loop/float(len(id)))}]  "),
	sys.stdout.flush()
